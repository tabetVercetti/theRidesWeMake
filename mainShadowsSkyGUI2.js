...
function init() {
    ...
    new RGBELoader()
        .setPath( 'Environment/' )
        .load( hdriMaps.Map1, function ( texture ) {
    ...
    loader1.load('TestCarVD4.glb',function(gltf){                    
        carModel = gltf.scene;
        gltf.scene.traverse(function(child){
            if(child.isMesh){
    ...
    toneMappingFolder.add( params, 'HDRItoneMapping', Object.keys( toneMappingOptions ) )

    .onChange( function () {

        updateGUI( toneMappingFolder );

        renderer.toneMapping = toneMappingOptions[ params.HDRItoneMapping ];
        render();
        //console.log('batata2')
...
function updateGUI( folder ) {
    ...
    if ( params.HDRItoneMapping !== 'None' ) {
