...
let scene, camera, renderer, composer, hlight, car, controls, orbitalControls, pointerLockControls, objectCenter, ground, carModel, wheel_FL, wheel_FR, wheel_RL, wheel_RR, body, glass,lights_rear, initialFrontWheelHeight, initialRearWheelHeight, initialBodyHeight, stats ; 
let test, test2;
let sky, sun;
...
const maxWheelVerticalTravel = 0.1; // value set by creator of each car
let roadBumpHeight;
...
const roadBumpCheck = 100; // sample distance for checking if road has bump or not
...
function init() {
    ...
    const groundMat = new THREE.MeshPhysicalMaterial( {
        color: 0x6c8093, metalness: 0.0, roughness: 0.0, clearcoat: 0.0, clearcoatRoughness: 1.0
    } );
    ...
    loader1.load('TestCarV10.gltf',function(gltf){                    
        carModel = gltf.scene;
    ...
    const opDirLight = new THREE.DirectionalLight( 0xffffff, 3 );
    opDirLight.color.setHSL( 0.1, 1, 0.95 );
	opDirLight.position.set(  1, 1.75, - 1 );
    scene.add (opDirLight);
    ...
    const hemLight = new THREE.HemisphereLight( 0xffffbb, 0x080820, 1 ); //first color is Skycolor, second is ground color)
    scene.add( hemLight );

    // // Ambient Light 

    // const ambLight = new THREE.AmbientLight( 0xffffbb );
    // scene.add( ambLight );
    ...
    //sky.material.uniform.mieCoefficient.value = 0.005;
    //sky.material.uniform.mieDirectionalG.value = 0.7;
    scene.add( sky );
