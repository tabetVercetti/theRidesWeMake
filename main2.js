...
let scene, camera, renderer, composer, hlight, car, controls, orbitalControls, pointerLockControls, objectCenter, ground, carModel, wheel_FL, wheel_FR, wheel_RL, wheel_RR, body, lights_rear, initialWheelHeight, initialBodyHeight ; 
...
return noise * roadBumpAmplitude; // Scale the output to be between -2 and 2
    }
...
const brakeLightMaterial = new THREE.MeshBasicMaterial({
    ...
    emissiveIntensity: 1, // Intensity of the red glow
  });
...
const maxRoadBumpAmplitude = 0.01
let roadBumpAmplitude //= 0.01; // Adjust the intensity of the suspension effect
...
function init() {
    ...
    new RGBELoader()
        ...
        .load( 'kiara_1_dawn_4k.hdr', function ( texture ) {
            ...
            loader1.load('TestCarV7.gltf',function(gltf){                    
                carModel = gltf.scene;
                scene.add(carModel);
                ...
                initialWheelHeight = COG_wheel_FR.y;
                console.log(initialWheelHeight);
    ...
    // Create a new <div> element to display the car acceleration
    const carAccel = document.createElement('div');
    carAccel.id = 'car-accel';
    carAccel.style.position = 'absolute';
    carAccel.style.top = '50px';
    carAccel.style.left = '10px';
    carAccel.style.color = 'white';
    document.body.appendChild(carAccel);

    // Create a new <div> element to display the car speed
    const carSpeed = document.createElement('div');
    carSpeed.id = 'car-speed';
    carSpeed.style.position = 'absolute';
    carSpeed.style.top = '70px';
    carSpeed.style.left = '10px';
    carSpeed.style.color = 'white';
    document.body.appendChild(carSpeed);
    ...
    OCinit();
        ...
        // const bloomPass = new UnrealBloomPass( new THREE.Vector2( window.innerWidth, window.innerHeight ), 0.2, 0.1, 0.9 ); // strength, radius ,threshold
...
function animate() {
    ...
    // Update the car Accel display
    const carAccel = document.getElementById('car-accel');
    carAccel.textContent = `Car Acceleration: ${wheelAcceleration.toFixed(2)} `;
    // Update the car speed display
    const carSpeed = document.getElementById('car-speed');
    carSpeed.textContent = `Car Speed: ${wheelRotationSpeed.toFixed(2)} `;
    ...
    if (isdriving)
    {
        ...
        // // Bumps on Road
        // const roadBumpDisplacement = roadBumpAmplitude * Math.sin(2 * Math.PI * roadBumpFrequency * time);
        // wheel_FR.position.y = initialWheelHeight + roadBumpDisplacement;

        // Example usage:
        // Update x value to change the Perlin noise over time
        roadBumpAmplitude = maxRoadBumpAmplitude * (wheelRotationSpeed / maxWheelRotationSpeed)
        // x_perlin_bump += 0.01;
        // x_perlin = perlinNoiseBump(x_perlin_bump, seed_perlin);
        x_perlin += 0.005; // frequency of perlin noise , aka oscillation speed
        const perlinNoise = perlinNoiseRoad(x_perlin, seed_perlin);
        wheel_FR.position.y = initialWheelHeight + perlinNoise; // Displaces the wheel vertically
        wheel_FL.position.y = initialWheelHeight + perlinNoise; // Displaces the wheel vertically
        wheel_RR.position.y = initialWheelHeight + perlinNoise; // Displaces the wheel vertically
        wheel_RL.position.y = initialWheelHeight + perlinNoise; // Displaces the wheel vertically
        ground.position.y = perlinNoise // Displaces the ground vertically
