...
let scene, camera, renderer, hlight, car, controls, orbitalControls, pointerLockControls, objectCenter, carModel, wheel_FL, wheel_FR, wheel_RL, wheel_RR ; 
...
function PLCinit() {
    ...
    console.log('im in PLC Init function')


    // document.addEventListener('keydown', function(event) {
    //     if (event.key === 'Escape') {
    //         if (paused && pointerLockControls.enabled) {
    //             paused = false;
    //             pressedESCtolock = true;
    //             setTimeout(function() {
    //               pressedESCtolock = false;
    ...
    //             // } 
    //             // else if (pointerLockControls.enabled) {
    //             //     paused = false;
    //             //     pointerLockControls.lock();
...
function init() {
    ...
    new RGBELoader()
        ...
        .load( 'kiara_1_dawn_4k.hdr', function ( texture ) {
            ...
            loader1.load('TestCarV4.gltf',function(gltf){                    
                carModel = gltf.scene;
                scene.add(carModel);
            ...
            // // Traverse the hierarchy and find the wheels
    ...
    document.addEventListener('keydown', function(event) {
        if(!paused){
            switch (event.key) {
                ...
                // case 'm':
                //     // increase driving speed
                //     if (wheelRotationSpeed < 1) {
                //         wheelRotationSpeed += 0.1;
                //     }
                //     break;
                // case 'n':
                //     // Decrease rotation speed by 1, with a minimum of 0
                //     if (wheelRotationSpeed > 0.11) {
                //         wheelRotationSpeed -= 0.1;
                //     }
                //     break;
                default:
    ...
    window.addEventListener('keydown', (event) => {
        if (event.key === 'm') {
            wheelAcceleration = 0.01; // Adjust as needed
        }
    });
    ...
    document.addEventListener('keydown', function(event) {
        if(!paused){
            if (event.key === 'p' && controls === pointerLockControls) {
                ...
                // camera.position.set(0, 0, z); // set z to the desired distance from the camera to the origin
                // camera.lookAt(0, 0, 0); // optional: make the camera look at the origin
            }
        }
    });
...
function animate() {
    ...
    if ( pointerLockControls.enabled && pointerLockControls.isLocked === true ) {
        ...
        // if(paused){
        //     delta = 0 // disable moving while paused
        // } else 
        if (sprint) {
...
cameraFOVDiv.textContent = `Camera FOV: ${camera.fov.toFixed(2)} \u00B0`;
    ...
    console.log(wheelRotationSpeed);
}
